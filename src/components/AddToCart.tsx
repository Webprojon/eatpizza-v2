    ...
    <>
      <div className="flex rounded-sm my-6 border border-gray-200 dark:border-slate-700">
        {volumesToMap.map((volume: number, idx: number) => {
          const isSelected = selectedVolume[index] === volume || (idx === 0 && !selectedVolume[index]);
          const volumeLabel =
            isSelected ? `${volume} ${product.itemCategory === "pizza" ? "SM" : product.itemCategory === "drink" ? "L" : "ML"}` : volume;
          ...
          return (
            <div
              ...
              onClick={() => handleChangeItemVolume(index, volume, product.itemCategory)}
            >
              {volumeLabel}
            </div>
          );
